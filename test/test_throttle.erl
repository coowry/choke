...
check_peek_test_() ->
    [
     ...
     ?_test(begin
		...
		?assertEqual({warning, 2, 1000}, throttle_context:check(Context, Counter)),
     ...
     ?_test(begin
		...
		?assertEqual({warning, 2, 1000}, throttle_context:check(Context, Counter1)),
     ...
     ?_test(begin
		...
		?assertEqual({warning, 2, 1000}, throttle_context:check(Context1, Counter)),
...
restore_test_() ->
    [
     ...
     ?_test(begin
		...
		?assertEqual({ok, 1},  throttle:check(Context, Counter)),
		?assertEqual({ok, 2},  throttle:check(Context, Counter)),
		?assertEqual({warning, 2, 1000},  throttle:check(Context, Counter)),
		?assertEqual({ok, 0},  throttle:restore(Context, Counter)),
	        ?assertEqual({ok, 1},  throttle:check(Context, Counter)),
		throttle:stop(Context),
		throttle:stop()
     ...
     ?_test(begin
		...
		?assertEqual({ok, 1},  throttle:check(Context2, Counter)),
		?assertEqual({ok, 2},  throttle:check(Context1, Counter)),
		?assertEqual({warning, 2, 1000},  throttle:check(Context1, Counter)),
